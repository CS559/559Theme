...
any consumer site (or transitively by a live theme template). This is an
information map, not a delete list: 559Theme has consumers outside this
workspace's sites, so "zero usage here" is a soft-retire candidate to
hand-review, not evidence it's safe to delete (see docs/deprecation.md).
Read-only: writes NOTES-usage.md, deletes nothing.
...
SITES = ["765-25", "559-sp26", "VisSnacks", "559Tutorials", "gleicher.github.io"]
# short, readable column headers for sites whose repo name is long
SITE_LABELS = {"gleicher.github.io": "gleicher"}
...
def main():
    ...
    out.append("Generated by `tools/usage-matrix.py`. Read-only information map, "
               "**not a delete list** — 559Theme has consumers outside these "
               "sites, so zero usage here is a soft-retire candidate to hand-review "
               "(see docs/deprecation.md), not evidence it's safe to delete. "
               "Columns are the in-workspace consumer sites; numbers are literal "
               "call-site counts (shortcodes) or reachability (partials).\n")

    out.append("## Shortcodes (call sites in content/assets/layouts)\n")
    out.append("| shortcode | " + " | ".join(SITE_LABELS.get(s, s) for s in SITES) + " | total |")
    out.append("|---" + "|--:" * (len(SITES) + 1) + "|")
    for name, counts, total in sc_rows:
        flag = " **← DEAD**" if total == 0 else ""
        cells = " | ".join(str(counts[s]) for s in SITES)
        out.append(f"| `{name}` | {cells} | {total}{flag} |")
